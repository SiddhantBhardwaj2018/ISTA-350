...
def get_soup(url = None,fname = None,gzipped = False):
	'''
	This function takes in 3 arguments url and fname which defaults to None and
	gzipped which defaults to False. If the fname is not None, it returns a 
	BeautifulSoup Object which opens the filename.If url is None,it raises a Runtime 
	Error else server = requests.get(url). If gzipped is true, it decompresses the file and 
	returns the BeautifulSoup object for the new variable.
	'''
    if fname:
...
def save_soup(fname,soup_obj):
	'''
	This function writes a the textual representation of a soup object to a text file.
	'''
...
def main():
	'''
	This main function checks if the check_file is present in the directory and if it is not there, it scrapes and 
